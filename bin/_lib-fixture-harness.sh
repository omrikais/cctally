...
#
# Setup-time analogs of bin/build-release-fixtures.py's _emit_fake_npm and
# _emit_fake_brew_tap. Scenario setup.sh files invoke these from cwd =
# $work/private/. Duplication with the Python emit helpers is intentional —
# setup.sh runs at scenario-execution time, not builder time.
# ---------------------------------------------------------------------------

fixture_seed_package_json () {
    cat > private/package.json <<'EOF'
{
  "name": "cctally",
  "version": "0.0.0-managed-by-release"
}
EOF
    ( cd private && git add package.json && git -c user.email=fake@test.local -c user.name=Fake commit -q -m "seed package.json" && git push -q origin HEAD ) || true
}

fixture_init_fake_npm_path () {
    mkdir -p ../PATH
    cat > ../PATH/npm <<'EOF'
#!/usr/bin/env bash
set -uo pipefail
