...
def build(out: pathlib.Path) -> None:
    ...
    # Pin the terminal width so the Codex-report terminal tables are byte-stable
    # regardless of the ambient terminal. bin/_lib_render.py sizes columns from
    # os.get_terminal_size(), falling back to $COLUMNS/120; a real terminal
    # (detected on the macOS runner) is wider than ubuntu CI's no-TTY fallback,
    # so the compressed 8-column populated tables diverge across machines. Pin
    # both the fallback ($COLUMNS) and the primary path (os.get_terminal_size,
    # which ignores $COLUMNS when a terminal is attached) to 120 — the width the
    # committed goldens were generated at — so generation is machine-independent.
    previous_columns = os.environ.get("COLUMNS")
    os.environ["COLUMNS"] = "120"
    previous_get_terminal_size = os.get_terminal_size
    os.get_terminal_size = lambda *_a, **_k: os.terminal_size((120, 24))
    # Pin the CHANGELOG-derived version stamp so the branded md/html/svg goldens
    # (cctally_version frontmatter + "Generated by … · vX.Y.Z" footer) do not
    # drift on every release. _lib_share_templates._release_version() reads the
    # latest CHANGELOG entry via _cctally_core.CHANGELOG_PATH; point it at a
    # fixed 9.9.9 stub, mirroring the share-v2 harness (build-share-v2-fixtures.py).
    changelog_dir = tempfile.mkdtemp(prefix="cctally-sa-changelog-")
    stub_changelog = pathlib.Path(changelog_dir) / "CHANGELOG.md"
    stub_changelog.write_text("## [9.9.9] - 2026-05-09\n", encoding="utf-8")
    previous_changelog_env = os.environ.get("CCTALLY_TEST_CHANGELOG_PATH")
    os.environ["CCTALLY_TEST_CHANGELOG_PATH"] = str(stub_changelog)
    _restore_changelog_attr = None
    try:
        if out.exists():
            shutil.rmtree(out)
        out.mkdir(parents=True)
        module = _load_cctally()
        # Import caching: _cctally_core.CHANGELOG_PATH is resolved from the real
        # repo at import time, before the env override above can take effect, so
        # override the resolved attribute directly. _release_version() re-reads
        # it at call time, so this is what actually pins the stamp.
        import _cctally_core  # noqa: PLC0415
        _restore_changelog_attr = _cctally_core.CHANGELOG_PATH
        _cctally_core.CHANGELOG_PATH = stub_changelog
        ...
        if _restore_changelog_attr is not None:
            import _cctally_core  # noqa: PLC0415
            _cctally_core.CHANGELOG_PATH = _restore_changelog_attr
        if previous_changelog_env is None:
            os.environ.pop("CCTALLY_TEST_CHANGELOG_PATH", None)
        else:
            os.environ["CCTALLY_TEST_CHANGELOG_PATH"] = previous_changelog_env
        shutil.rmtree(changelog_dir, ignore_errors=True)
        os.get_terminal_size = previous_get_terminal_size
