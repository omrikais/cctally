...
When a mid-pass invalidation forces the segment-elision refill,
`quotaCacheCoverage.elisionRefill.failures` is an ordered list of
`{segment, error}` objects naming every segment that raised and its exception
class. An empty list means the refill caught no exception.
