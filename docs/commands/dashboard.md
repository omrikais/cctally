...
> `/api/conversation/search` and `/api/conversation/<id>/payload` are both
> matched **before** the `/api/conversation/<id>` reader, so neither `search`
> nor a `…/payload` suffix is ever treated as a session id. `/api/data` carries
> a per-request `transcriptsEnabled` boolean (the same gate value) so the
> client only offers the conversation UI when the routes would actually serve.
