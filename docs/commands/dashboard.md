...
The Current Week / Current Cycle modal (opened with `1`, or by clicking the hero strip) navigates history in place. The week/cycle chip is flanked by `‹`/`›` buttons that step to previous and back toward the current 7-day billing cycle; within a selected cycle, a block navigator steps between that cycle's known 5-hour blocks. It applies to both providers — Claude subscription weeks and Codex native quota cycles — and, in the side-by-side **All** view, each provider section navigates independently (click-only). The default view is always the current cycle with its active block.

In the single-provider variants, `ArrowUp`/`ArrowDown` step to the newer/older week and `ArrowLeft`/`ArrowRight` to the older/newer block. A week that contains a re-anchoring (goodwill) credit renders the full chronological ledger — every segment in crossing order with a full-width credit divider between them and cumulative cost restarting per segment. Weeks with no recorded milestones show an explicit empty state; cycles with no retained 5-hour data hide the block navigator. Historic weeks are read-only, so the Share action is hidden while one is displayed (it always shares the current week otherwise).

A compact per-provider navigation index rides the live SSE envelope (`current_week.week_index` for Claude, `sources.codex.data.quota.cycle_index` for Codex), built only on non-idle snapshot rebuilds. The complete payload for one selected week/cycle — its milestone segments, credit dividers, and 5-hour blocks — is fetched on demand from:

```
GET /api/milestones/<source>/week/<key>
```

`<source>` ∈ `claude | codex`. For Claude the `<key>` is the week's `week_start_date` (`YYYY-MM-DD`); for Codex it is an opaque, server-issued cycle key (use only keys obtained from the envelope index — never hand-constructed). The response is snake_case JSON with `Cache-Control: no-cache`; the client reuses it client-side keyed by `(source, key, detail_stamp)`. A malformed key or source returns `400`; a key that no longer resolves returns `404` with a machine-readable `{ code: "unknown_key", reason }` body (`reason` ∈ `pruned | rebuild_pending | projection_incoherent | unknown`).
