...
> `/api/conversation/<id>/media`, and `/api/conversation/<id>/find` are all
> matched **before** the `/api/conversation/<id>` reader, so neither `search`
> nor a `…/payload` / `…/media` / `…/find` suffix is ever treated as a session
