...
| Key | Action |
|---|---|
| `1`–`9` | Open the grid card at that position in your persisted order (default order: Sessions, Trend, Projects, Daily, Cache Report, Weekly, Monthly, Blocks, Forecast). Under Codex or All, the Sessions and Blocks positions open the top row's own detail modal rather than the panel's modal |
...
The `Blocks` panel marks a Claude row as retained when its cost and model
figures come from the facts saved when that five-hour block closed. The marker
appears on every such row, even when the dashboard clock places that closed
block inside the current five-hour window and also calls it Active. Rows
calculated from the current local cache have no retained marker; the panel
does not recalculate retained totals just to decide whether they differ.

Open a retained row for the block detail. Its cost and token statistics carry
an explanation for screen readers as well as visible text: the headline uses
the facts retained at close, while the trajectory is drawn from entries
currently in the cache. Those two totals can differ. The trajectory and its
projection are not a continuation of the retained headline.
...
The Current Week / Current Cycle modal (opened by clicking the hero strip, or by focusing it and pressing Enter or Space — the digits address the ten grid cards, so none of them opens the hero) navigates history in place. The week/cycle chip is flanked by `‹`/`›` buttons that step through provider billing cycles bounded by actual reset or re-anchor events—not calendar weeks or nominal seven-day buckets. A reset starts a new navigable cycle even when it arrives before the nominal deadline. It applies to both providers, and in the side-by-side **All** view each provider section navigates independently, with its own Older/Newer buttons. Those buttons are ordinary `<button>` elements, so a keyboard activates them as it activates any button; what the All view does not bind is the single-provider `ArrowUp`/`ArrowDown` cycle shortcuts, because each section owns its own navigation state and receives `EMPTY_BINDINGS`.

Within a selected cycle, the block navigator contains every retained five-hour block whose interval overlaps that cycle's exact half-open interval, ordered oldest to newest. It is visible immediately for both current and historic cycles: the current cycle defaults to its active block, while a historic cycle defaults to its last retained block. `ArrowLeft`/`ArrowRight` or the block buttons move one block per action, including when the first action lazily fetches detail before applying its direction. Boundary-straddling and milestone-empty blocks remain navigable. A cycle with no retained five-hour data hides the navigator as honest data absence; Codex automatically gains the same navigator whenever retained native 300-minute observations exist.

For Claude, the live five-hour block follows the account that supplied the displayed weekly percentage. Its credits and crossings use that same account. When more than one Claude account is registered, the live block includes its account identity so a five-hour window shared by two accounts remains distinct while browsing history. The live block stays visible while the account-aware block order loads; its arrows become available when that detail arrives. A failed detail request leaves the live block visible with a Retry action. With one account, the block keeps its existing wire shape.
...
For a bare Claude session with no retained transcript, `GET /api/conversation/<id>/outline?progressive=1` returns HTTP 200 `{"status":"not_found"}` without a transfer ticket. The non-progressive outline URL and a missing qualified `v1.` outline retain their HTTP 404 response. Session detail still renders its accounting fields.
...
S4 made the dashboard server publish Claude, Codex, and presentation-only
`all` states atomically. S5 added the visible source selector and browser
interaction over that contract. The source-aware endpoints above remain the
backend contract the client reads.
...
**Every cross-provider ranking states the range it covers, and All keeps two range rules at once.** A combined **total** sums each provider's own current cycle, as the paragraph above describes. A cross-provider **ranking or comparison** — the Projects leaderboard, the combined Daily heatmap — uses one shared absolute calendar range instead, resolved once and applied identically to both providers, and the panel names its resolved dates. Both rules are deliberate: a total is only reconcilable against each provider tab when each leg covers that provider's own billing period, and a ranking is only fair when both legs cover the same interval. Before this, the Projects panel concatenated a Claude leaderboard bounded by the subscription week with a Codex leaderboard bounded by roughly thirty days and printed "this week" over the result, so a Codex project could outrank a Claude project because of the window rather than the cost.

The percentage beside each project is named in the panel legend: a share of the week's quota on the Claude tab, a share of the ranked cost on Codex and All. A row the drill-down cannot reach — a project active between a week's rollover and the projects envelope re-anchoring is ranked but not routable — stays in the ranking at its real cost and simply offers no detail view, rather than opening a modal that fails.

Under All, Weekly and Monthly render one section per provider and foot a combined cost with the span it covers and each provider's share. Weekly names exact dates, which its rows carry; Monthly names a month-label span, because monthly rows carry no bounds and the current month's read ends at the current instant rather than at the month's end. Daily sums both providers per calendar day, and the day's detail breaks that sum back down per provider; the merged row publishes no cache-hit percentage and no model split, because Claude and Codex count cached input differently and one model stack over two vendors' families would share a denominator that means nothing. Blocks interleave both providers' five-hour windows in time order and state the interval the shown windows span — never a shared reset cycle, because the two providers' five-hour clocks are independent. Recent Sessions lists both providers' sessions in one table rather than in per-provider sections, with a source chip on each row, and states that composition beneath its title.

When the shared range or a provider's contribution to it cannot be resolved, the panel says so and names the missing fact. That state is distinct from "no activity yet" and from "restart the dashboard": it reports a range problem as a range problem instead of as emptiness or as a broken instance.

Every source selection owns the same canonical ten-card board: capability gating never removes a panel and reflows the bento, and a capability a source does not publish renders inside its own card as an honest empty or unavailable state. The Help overlay (`?`) therefore lists no omissions, because there are none to list; Codex reset forecasts also feed the canonical hero, while its computed token-reuse detail lives in Cache Report. A runtime-degraded or stale source renders an explicit warning chip in place (never hidden), and a source with no successful snapshot yet renders "no successful snapshot yet". A fresh partial source degrades only the warning's affected domain (for example, incomplete project metadata leaves Daily and quota usable); a source-wide ingest/read-model warning degrades every applicable domain. The source-status chip uses concise visible copy while preserving its full diagnostic in its title and accessible label.

There is one persisted ten-card order, and the digit shortcuts, the drag-reorder and Help's panel list all read it directly. It is not filtered by the active source, no panel is ever hidden from it, and there is no hidden-panel remapping; a source switch never rewrites it.
...
**The Forecast panel and its modal carry the quota model's own view of the week (#661 S2 §10).** The panel adds one footer line naming the basis its projection came from — `model`, `meter`, or the withholding cause in the short register — and a `Δ rate` chip while the **merged** active metering regime has a confirmed predecessor. That read is account-blind, exactly as [`doctor`](doctor.md)'s `quota.meter_drift` check is and as the [status line](statusline.md)'s `Δrate` marker is, so on a decorated multi-account install the chip states that a metering rate changed rather than that the focused account's did. Within one `forecast.quota` object the projection half IS account-scoped and this half is not; `cctally quota` is the account-scoped view. The modal adds a Quota model section with the modelled consumption and headroom, what the meter reading covers after the ceiling correction, and two separate cause fields: one for a withheld projection, one for why the calibrated basis was not reached. Falling back to the meter is not a withholding, so a single field cannot answer both, and a surface saying nothing in the second is silently falling back.

**The observed-minus-modelled difference appears in the modal and nowhere else.** It is labelled strictly as the observed meter minus the modelled local quota, it carries the sentence saying it does not identify or estimate usage from another machine, and it is withheld unless a single resolved account, whole subscription weeks and an unsplit population all hold. It stays out of the headline, the status line, `doctor`, project normalization and every dollar figure.

**There is no presentation latch.** Trust is re-evaluated on every refresh and the surface switches with the shared selector, always exposing the active basis and its cause. A latch would either keep presenting a model after its evidence was withheld, or require durable state this session may not add.

**Metering-rate changes render as their own family, never as an alert row.** Every member of the alert axis registry is a numeric threshold axis, and a rate transition has no threshold and no threshold-derived severity, so it travels in its own wire array with its own discriminated variant. The Recent Alerts modal gives it a separate section whose columns are the ones it actually has — provider, the size and direction of the change, the units-per-point pair, and the effective instant — and the toast branches on the variant tag rather than being forced through the threshold body. Recording is unconditional from the first upgrade; the push follows the default-off `alerts.rate_change_enabled` toggle, which the Settings overlay renders under Alerts.
...
representation-specific `ETag`, and send `Vary: Accept-Encoding`; a warm valid
`304` revalidates the shell without rereading or recompressing it. A warm reload
