...
> `/api/conversation/search`, `/api/conversation/<id>/payload`, and
> `/api/conversation/<id>/media` are all matched **before** the
> `/api/conversation/<id>` reader, so neither `search` nor a `…/payload` /
> `…/media` suffix is ever treated as a session id. `/api/data` carries a
