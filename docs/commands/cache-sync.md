...
After it deletes an orphan's rows, the prune re-derives the `conversation_sessions` rollup for the affected sessions. That re-derive is refused in any of three states, and the stderr line names the one that actually occurred rather than assuming a version difference. The ordinary state is a running process holding an older embedded pricing table than the one the store recorded, which happens most often when an older binary runs against a store a newer one has already written. The second is a recorded fingerprint that is not a date cctally can order against its own. The third is a fingerprint cctally could not read at all, where it has no evidence about what the store's cost needed protecting from and refuses rather than assuming there was nothing to protect. In every one of them the deletions themselves still commit and the rollup backfill is armed for the next authorized process, but the store is left partially derived, so the command reports the refusal, names the number of affected files, and **exits 3** — a staged failure under `docs/cli-contract.md`, not a usage error. `cctally doctor pricing.conversation_rollup_writer` names the step that clears each state; run the prune again once an authorized process can complete it.

Three outcomes, in precedence order:

| Outcome | Exit | Why it ranks here |
|---|---|---|
| The cache lock is held by another process | 1 | Reported first because nothing was attempted at all. |
| The rollup re-derive was refused | 3 | Outranks a residual report: committed deletions were not followed by an authorized re-derive. |
| Orphans left in place as residuals, or a clean prune | 0 | A residual is work deliberately not done, and `cache-sync --rebuild` clears it. |
...
- `--rebuild` also exits non-zero when cctally cannot prove its embedded pricing table is at least the one recorded in the conversation store. cctally refuses to rewrite materialized conversation cost from a pricing table it cannot order against the store's, and a rebuild is exactly that write, so it declines the whole operation rather than emptying the rollup and then being refused the re-derive. Three states produce this, and only the first is about a pricing version. The ordinary one is a recorded fingerprint **newer** than this cctally's; run the newer cctally, or restart the process that recorded the newer table. The second is a recorded value that is **not an ISO date**, which no version can order — an upgrade or a restart reaches nothing there, and neither does another `--rebuild`. The third is a fingerprint the pre-check could **not read at all**, because the `cache_meta` SELECT raised: the pre-check uses the fail-closed predicate, so a read that did not happen refuses rather than being treated as a store with nothing to protect, and the step is to release whatever is holding the store and try again. All three name `phase=pricing`, and `cctally doctor` separates them under `pricing.conversation_rollup_writer`, whose remediation names the step that clears each one. A routine (non-`--rebuild`) sync still reports success in any of those states, because message ingestion did succeed there and only the rollup re-derive was declined.
