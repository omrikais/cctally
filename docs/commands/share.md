...
The dashboard share GUI stamps and displays source identity end-to-end. Opening a share flow (a panel's Share affordance, or `Shift+S` with one of that panel's actions focused) captures the active source at that moment; switching the global selector while the modal, preview, or export is open never restamps the in-flight flow. Every render and composer request the client issues carries an explicit `source` (including `claude`) — so newly produced artifacts and history entries visibly say "Claude", "Codex", or "All" (an intended, documented change; a never-switching Claude user's data and flows are otherwise unchanged). The modal's live preview surfaces the flow's source label chrome so the pre-copy preview matches what the artifact says.

A share taken while an account is focused carries that account's qualification, and that now includes shares taken under `All` (#556 S5). Opening a share flow under `All` used to capture no account at all, so a Codex chip could read as one account while the artifact it produced contained every account's data. The capture reads the `All` view's own Codex focus — Codex is the only provider whose share body the server can narrow — and a Claude focus under `All` qualifies nothing, because Claude publishes no per-account projections for the server to narrow. The Codex Forecast artifact also states the configured budget beside its quota projections, so a shared Forecast says what the panel it was taken from says; the line is omitted entirely when no budget is configured.

The share picker and the `Shift+S` shortcut gate through the per-source panel matrix (`SHARE_PANEL_MATRIX`). All three selections — Claude, Codex and `All` — currently hold the SAME nine panels (`current-week`, `trend`, `weekly`, `daily`, `monthly`, `blocks`, `forecast`, `sessions`, `projects`), so the gate rejects nothing today; it is the enforcement point for any future per-source divergence. An earlier version of this paragraph said forecast and trend were absent under Codex and `All`. They are not, and never were.
