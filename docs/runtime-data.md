...
All persistent state lives under `~/.local/share/cctally/` (a dev checkout uses `~/.local/share/cctally-dev/` instead). Nothing here is committed to the repo. Most of it re-derives on demand. When cctally has retained journal data for this installation, `stats.db` is a disposable index derived from the append-only journal under `journal/`: cctally rebuilds or heals it automatically, and deleting it loses nothing. On a pre-cutover installation with no retained journal data, `stats.db` may be the only copy of your recorded history — cctally preserves it, refuses to replace it with an empty rebuild, and points you at guarded repair instead. A few files (`config.json`, `install_id`) "regenerate" only to fresh defaults — the prior value is gone. The file map below is explicit about which is which.

## File map

| Path | Regenerated automatically? | What is lost if deleted? |
| --- | --- | --- |
| `stats.db` | Yes, when journal data is retained — automatically on damage, or on demand with `cctally db rebuild --db stats` | Nothing on an installation with retained journal data: it is a disposable index and every row in it — usage snapshots, cost snapshots, and the percent / 5-hour / budget milestones, reset events and credit floors derived from them — is rebuilt from the journal. On a pre-cutover installation with no retained journal data it may be the only copy of that history, so cctally preserves it, refuses to replace it with an empty rebuild, and points you at `cctally db repair --db stats --yes` instead. |
| `stats.db-journal` | Yes — SQLite auto-manages the transient rollback journal | Nothing after a clean commit or rollback. Never move or delete it while a process has `stats.db` open; physical recovery preserves it with the main file after proving the family is cold. |
| `stats.db-wal`, `stats.db-shm` | Legacy transition artifacts only | Current binaries never create these. If either exists, stop/restart older dashboards and cctally processes and retry the epoch rebuild; cctally leaves the family untouched until every legacy holder drains. |
| `cache.db-wal`, `cache.db-shm`, `conversations.db-wal`, `conversations.db-shm` | Yes — SQLite auto-manages them | Nothing once the owning DB is closed cleanly (they checkpoint back into the parent `.db`). Deleting them under a live writer can drop the most recent uncheckpointed writes. |
...
| `quarantine/cache.db-*`, `logs/cache.db-corruption-forensics-*` | No — retained incident evidence, bounded by `storage.artifact_retention` | A confirmed damaged SQLite family and its diagnostic metadata. An unconfirmed classified trigger creates only the forensics bundle and leaves the live family byte-for-byte in place; preserve either artifact when investigating recurring failures. Reclaim with `cctally db prune`, which **never deletes** evidence whose cause cctally could not determine. |
| `quarantine/stats.db-*` | No — retained incident evidence, bounded by `storage.artifact_retention` (30 days / 20 per family / 4096 MiB by default) | The preserved `stats.db` family from one corruption incident, plus the `manifest.json` recording what triggered it and, once the classifier has run, a `classification.json` recording the cause. Nothing is lost by deleting one: the live index was already rebuilt from the journal. Reclaim with `cctally db prune`. An incident whose cause cctally could not determine, one something is still using, and the newest surviving example of each distinct kind of damage — one example per kind, for as many kinds as `max_shape_examples` allows (8 by default) — are **never deleted automatically**. |
| `logs/stats.db-corruption-forensics-*.json` and the matching `logs/stats.db-corruption-forensics-*/` directory | No — retained incident evidence, bounded by the same policy | The diagnostic bundle captured by the elected recovery owner after maintenance readers drain and before it may mutate the family, and — when there was a non-empty write-ahead log — a copy of the `-wal`/`-shm` sidecars as they stood. A bundle referenced by a rebuild record or a quarantine incident is reclaimed together with it; an unreferenced one is reclaimed on its own. |
| `logs/stats-rebuild-*.json` | No — retained incident evidence, bounded by the same policy | One record per stats index rebuild: why it ran, which mechanism published it, and the post-publication verdict. `cctally db prune` keeps a record whose rebuild has not reached a terminal status, because that is a rebuild still in flight. |
| `logs/stats-heal-events.json` | Yes — a fixed-size ring, capped at 50 entries at write time | The recent auto-heal occurrence history `cctally doctor`'s `journal.auto_heal` check reports. Each admitted occurrence counts its coalesced detectors in one entry and receives the background owner's terminal outcome. It needs no reclamation and is not part of the retention corpus. |
| `quarantine/stats.db-*/classification.json`, `*.bak-*.classification.json` | Yes — rewritten whenever the cause is re-determined | cctally's verdict about what caused one incident, or what wrote one machine-made backup. Deleting it does not lose the evidence, but it makes that evidence unclassified, and unclassified evidence is **never deleted automatically** — so it will be retained until you classify or remove it by hand. |
...
| `stats.db.bak-*` | **No** — a point-in-time snapshot, in three shapes: `stats.db.bak-<timestamp>` written by `cctally db backup`, `stats.db.bak-corrupt-malformed-<timestamp>` preserved by `cctally db repair`, and any copy saved by hand under a name of its own | The snapshot of `stats.db` it holds. On an installation with retained journal data that costs you nothing, because the disposable index it copied is rebuilt from the journal; on a pre-cutover installation it may be the only copy of history the journal cannot supply. Retention sweeps `db repair`'s own copies; `cctally db backup` families need `cctally db prune --include-backups`, and a name cctally does not recognize is **never deleted automatically**. |
| `data.db`, `usage.db` | n/a — legacy | Files from earlier iterations with no current writer. Safe to remove. |

## `stats.db` schema

A **disposable index** materialized from the append-only journal (`~/.local/share/cctally/journal/`), which is the durable truth since the DB journal redesign (§7.1). `stats.db` is stamped at a single `STATS_INDEX_EPOCH` (1010) rather than versioned by migrations; its 13-migration legacy registry is **frozen** (only used to bring a pre-cutover install to the export baseline). A version mismatch — newer or older binary — self-heals by **rebuild from the journal** (`cctally db rebuild --db stats`); `DowngradeDetected`-bricking no longer applies to `stats.db`, and `db recover --db stats` is retired. Corruption self-heals the same way, with no human step. Since #530 the detecting process first elects one durable background owner and returns; that owner waits for maintenance readers to drain, captures forensics, confirms the damage, and replays the journal. The healthy rebuild is **published transactionally into the existing file**, so it retains the inode and leaves no quarantined copy. Physical replacement is reserved for an absent destination or one proven structurally corrupt/unopenable before live mutation, after a cold second drain and preservation of the complete old family. A schema change bumps the epoch, never adds a stats migration.
