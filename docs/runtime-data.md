...
| `logs/stats.db-corruption-forensics-*.json` and the matching `logs/stats.db-corruption-forensics-*/` directory | No — retained incident evidence, bounded by the same policy | The diagnostic bundle written the moment damage was detected, and — when there was a non-empty write-ahead log — a copy of the `-wal`/`-shm` sidecars as they stood. A bundle referenced by a rebuild record or a quarantine incident is reclaimed together with it; an unreferenced one is reclaimed on its own. |
...
| `logs/stats-heal-events.json` | Yes — a fixed-size ring, capped at 50 entries at write time | The recent auto-heal history `cctally doctor`'s `journal.auto_heal` check reports. It needs no reclamation and is not part of the retention corpus. |
...
A **disposable index** materialized from the append-only journal (`~/.local/share/cctally/journal/`), which is the durable truth since the DB journal redesign (§7.1). `stats.db` is stamped at a single `STATS_INDEX_EPOCH` (1009) rather than versioned by migrations; its 13-migration legacy registry is **frozen** (only used to bring a pre-cutover install to the export baseline). A version mismatch — newer or older binary — self-heals by **rebuild from the journal** (`cctally db rebuild --db stats`); `DowngradeDetected`-bricking no longer applies to `stats.db`, and `db recover --db stats` is retired. Corruption self-heals the same way, with no human step, but the flow changed in #496 S3: the process that detects the damage writes a forensics bundle, schedules a background rebuild and returns, rather than replaying the journal on its own thread. The rebuild is then **published transactionally into the existing file**, so it leaves no quarantined copy — quarantine is a consequence of destroying a file, and only the fallback path taken when the destination cannot be opened at all destroys anything. A schema change bumps the epoch, never adds a stats migration.
