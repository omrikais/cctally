  ...
  return (
    <div className="conv-cmp-head">
      ...
      <div className="conv-cmp-head-sides">
        <Side side={a} label="Run A" />
        <Side side={b} label="Run B" />
      </div>
...
function Side({ side, label }: { side: SideHeader; label: string }) {
  const meta = [side.date, side.model].filter(Boolean).join(' · ');
  return (
    <div className="conv-cmp-head-side">
      <span className="conv-cmp-head-side-tag" aria-hidden="true">{label}</span>
