...
describe('panel re-key on source switch (§5.1)', () => {
  it('Claude → Codex: no stale Claude-sourced text survives the switch frame', () => {
    ...
    render(<DailyPanel />);
    // Claude-only heatmap chrome is present under Claude. The descriptor and
    // the window are two elements since #556 S2 QA moved the window onto the
    // panel's wrapping sub-line, so the landmark reads both.
    expect(screen.getByText(/heatmap/)).toBeInTheDocument();
    expect(document.querySelector('#panel-daily .panel-range-note')!.textContent)
      .toBe('30 days');
    act(() => {
