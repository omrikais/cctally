...
describe('<HelpOverlay /> positional hotkey rule', () => {
  it('renders <kbd>1..9,0</kbd> for positions 1..10 (#264 S2 — 10 cards)', () => {
    render(<HelpOverlay />);
    openHelp();
    // #264 S2 — DEFAULT_PANEL_ORDER has 10 grid entries (the History card is
    // split back into Daily/Weekly/Monthly), so digits 1..9 then '0' for
    // position 10; no literal ≥11 em-dash row.
    ...
    // '0' (position 10) and '9' are present now; no literal <kbd>10</kbd>/<kbd>11</kbd>.
    expect(kbds.find((k) => k.textContent === '0')).toBeDefined();
    expect(kbds.find((k) => k.textContent === '9')).toBeDefined();
  ...
  it('renders exactly 10 panel rows (no ≥11 em-dash row) for the grid order', () => {
    render(<HelpOverlay />);
    openHelp();
    // #264 S2 — the grid order is 10 long: every panel row carries a digit
    // (1..9,0); the positional ≥11 em-dash branch stays dormant.
  ...
  it('documents the select-period key in the hand-written rows, with no Day/Week/Month toggle (#264 S2)', () => {
    render(<HelpOverlay />);
    openHelp();
    // #264 S2 — the ↑/↓ row now reads "Daily / Weekly / Monthly modal"; the
    // ←/→ Day·Week·Month toggle row was removed with the toggle.
    expect(screen.getByText(/Select period \(Daily \/ Weekly \/ Monthly modal\)/i)).toBeInTheDocument();
    expect(screen.queryByText(/Switch Day \/ Week \/ Month/i)).toBeNull();
  });
});
