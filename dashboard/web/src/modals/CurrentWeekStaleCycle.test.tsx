...
function staleCycleEnv(): Envelope {
  return envWith((b) => {
    ...
    // What compose_all_state emits alongside a withheld combined total: an
    // All-LOCAL warning, on the `all` source rather than either provider.
    b.sources.all.warnings = [{
      code: 'combined_totals_withheld',
      message: 'Codex quota evidence is stale, so combined totals are withheld.',
      domain: 'hero',
    }];
  });
}
...
describe('CurrentWeekModal — stale Codex cycle disclosure (#350)', () => {
  ...
  it('replaces the per-provider Codex note with the All-local reason on the All modal', () => {
    ...
    expect(reason!.textContent).toMatch(/combined totals are withheld/i);
  });
