...
describe('<HelpOverlay /> reflects panelOrder', () => {
  ...
  it('updates the labels after a reorder', async () => {
    ...
    render(<HelpOverlay />);
    await open(user);
    // Swap positions 0 and 1: default order is forecast(0), trend(1), so the
    // row with key=1 should now show "Trend" (the new occupant of index 0).
    ...
    expect(row1?.textContent).toContain('Trend');
  });
