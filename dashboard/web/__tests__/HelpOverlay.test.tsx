...
describe('<HelpOverlay />', () => {
  ...
  it('opens on ? and renders the keybindings table + meta server-url line', async () => {
    ...
    // 29 table rows: 10 panel keys (1-9,0 - #264 S2 split the History card back
    // into Daily/Weekly/Monthly, so the grid order is 10 long) + 15 data-driven
    // HELP_ROWS single-key rows (#207 D1: r, v, a, s, d, e, S, B, f, /, c, n/N, q,
    // ?, Esc - #294 S5 added `v` for the source cycle, #341 Task 4 added `a` for
    // the account cycle, #620 S2 added `e` for the diagnosis) + 4 combo/gesture
    // rows (Hold+drag, Shift+arrows, up/down select period, and the #556 S4 hero
    // row, which names the one modal no digit can reach).
