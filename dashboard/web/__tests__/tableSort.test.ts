...
import { ALL_SESSIONS_COLUMNS } from '../src/lib/sessionsColumns';
import type { SessionRow } from '../src/types/envelope';

describe('ALL_SESSIONS_COLUMNS registry (superset the store sorts through)', () => {
  it('exposes the seven-column superset in canonical order', () => {
    expect(ALL_SESSIONS_COLUMNS.map((c) => c.id)).toEqual([
  ...
  it('started comparator orders by parsed timestamp ascending', () => {
    const started = ALL_SESSIONS_COLUMNS.find((c) => c.id === 'started')!;
    const a: SessionRow = {
...
describe('TREND_COLUMNS registry', () => {
  it('exposes the five columns matching the trend table (S3 #264 added Cost)', () => {
    expect(TREND_COLUMNS.map((c) => c.id)).toEqual([
      'week', 'cost_usd', 'used_pct', 'dollar_per_pct', 'delta',
    ]);
  });
  ...
  it('cost_usd comparator orders by cost ascending; nullKey parks nulls (S3 #264)', () => {
    const cost = TREND_COLUMNS.find((c) => c.id === 'cost_usd')!;
    const a: TrendTableRow = {
      label: 'a', used_pct: null, dollar_per_pct: null, delta: null,
      is_current: false, _chronoIdx: 0, cost_usd: 10,
    };
    const b: TrendTableRow = { ...a, label: 'b', cost_usd: 50 };
    const c: TrendTableRow = { ...a, label: 'c', cost_usd: null };
    expect(cost.compare(a, b)).toBeLessThan(0);   // 10 < 50
    expect(cost.nullKey?.(c)).toBeNull();          // null cost → parks last
    expect(cost.nullKey?.(a)).toBe(10);
  });
