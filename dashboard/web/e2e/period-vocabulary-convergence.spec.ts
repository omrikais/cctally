...
for (const viewport of MATRIX) {
  test(`period vocabulary convergence and provider-native tokens at ${viewport.width}x${viewport.height}`, async ({ page }) => {
    ...
    // The PERIOD vocabulary is now shared, not provider-native, and this block
    // guards that convergence rather than a contrast.
    //
    // Claude counts in cycles exactly as Codex does, and #750 S4 D3 renamed the
    // period field to `Reset cycle` on this branch too: a Claude row has always
    // been a billing cycle, because an in-place quota credit ends one cycle and
    // begins another inside the same subscription week, and the field's VALUE is
    // the span the row covers. Both assertions below are therefore regression
    // guards against reintroducing the week-flavoured field name, not evidence
    // of a difference between the two providers.
    //
    // The axis that IS provider-native is the TOKEN vocabulary, which the two
    // directions immediately below assert in one place: Claude renders
    // `Cache+` / `Cache-read` and never Codex's `Cached input` / `Reasoning`,
    // and the Codex Daily and Monthly dialogs at the top of this test assert
    // the mirror image. The `all` composition takes a third noun that belongs
    // to neither provider — `provider periods`, `Provider period`, `vs prior
    // provider period` — because a composed row spans both quota systems and
    // can be named after neither.
    ...
    await expect(claudeWeekly.getByText(/Reset cycle:/)).toBeVisible();
    await expect(claudeWeekly.getByText(/Subscription window:/)).toHaveCount(0);
    ...
    for (const codexOnlyLabel of ['Cached input', 'Reasoning']) {
      await expect(claudeWeekly.getByText(codexOnlyLabel, { exact: true })).toHaveCount(0);
    }
