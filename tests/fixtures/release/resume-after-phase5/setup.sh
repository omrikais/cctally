...
cat > "$work/npm-mock-state.json" <<'CCTALLY_NPM_STATE_EOF'
{
  ...
  "publish": {
    "exit": 0,
    "stdout": "+ cctally@0.1.1"
  }
}
CCTALLY_NPM_STATE_EOF
CCTALLY_RELEASE_DATE_UTC=2026-05-07 GIT_AUTHOR_DATE="2026-05-07T00:00:00+0000" GIT_COMMITTER_DATE="2026-05-07T00:00:00+0000" PATH="$work/fake-bin:$PATH" GH_ARGV_LOG="$work/gh-argv.log.partial" GH_NOTES_DEST="$work/gh-notes.partial.txt" NPM_MOCK_STATE_FILE="$work/npm-mock-state.json" NPM_MOCK_LOG_FILE="$work/npm-invocations.log.partial" CCTALLY_RELEASE_BREW_ARCHIVE_URL="file://$REPO_ROOT/tests/fixtures/release/_assets/fake-archive-v1.0.1.tar.gz" python3 bin/cctally release patch --skip-brew > "$work/_partial.stdout" 2> "$work/_partial.stderr"
...
cat > "$work/npm-mock-state.json" <<'CCTALLY_NPM_STATE_EOF'
{
  ...
  "view": {
    "exit": 0,
    "stdout": "\"https://registry.npmjs.org/cctally/-/cctally-0.1.1.tgz\""
  },
  "publish": {
    "exit": 0,
    "stdout": "+ cctally@0.1.1"
