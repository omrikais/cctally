"""OK / GOOD state: 34.2% used, comfortable headroom.

Encodes the OK / GOOD state. The populated forecast carries the
DECELERATING rate case (r_recent < r_avg),
which is the scenario where the Variant B ribbon label-swap bug fixed in
commit b323f5e would be visible. Verdict resolves to GOOD — both the
week-avg and recent-24h projections land comfortably under 90%.
...
_P_NOW = 34.2
_P_24H = 31.0  # Decelerating: r_recent = (34.2 - 31.0)/24 ≈ 0.133 < r_avg ≈ 0.224
_DPP = 0.42

_R_AVG = _P_NOW / _ELAPSED_H                                      # ≈ 0.22353
_R_REC = max(0.0, (_P_NOW - _P_24H) / 24.0)                       # ≈ 0.13333
_P_FIN_AVG = _P_NOW + _R_AVG * _REMAINING_H                       # ≈ 37.553
_P_FIN_REC = _P_NOW + _R_REC * _REMAINING_H                       # ≈ 36.200
...
SNAPSHOT = m.DataSnapshot(
    ...
    trend=[
        ...
        m.TuiTrendRow("Apr 14", _WEEK_START, 34.2, 0.42, -0.07, 4, True),
    ],
    sessions=[
