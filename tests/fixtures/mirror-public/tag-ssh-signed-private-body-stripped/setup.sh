...
command -v ssh-keygen >/dev/null || { echo "SETUP_FAIL: ssh-keygen unavailable" >&2; exit 1; }
...
git cat-file -p v1.0.0 | grep -q "BEGIN SSH SIGNATURE" || { echo "SETUP_FAIL: private tag was not actually SSH-signed" >&2; exit 1; }
