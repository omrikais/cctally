...
from tests._support_http import PRESENCE_BACKSTOP_SECONDS, post_json, serve_dashboard, stop
...
def _post(host, port, path, *, origin_host=None, host_header=None):
    """POST {} with explicit Origin and Host headers, either of which may be absent.

    An absent Origin or Host is a negative case of the CSRF contract rather
    than an oversight, so an unsupplied one is omitted rather than defaulted.
    `post_json` omits any header mapped to None, which is what expresses that.
    """
    status, _body = post_json(
        port, path, {}, host=host,
        headers={
            "Origin": f"http://{origin_host}" if origin_host else None,
...
def test_lan_bind_lan_ip_origin_match(monkeypatch, tmp_path):
    ...
    redirect_paths(ns, monkeypatch, tmp_path)
    _wire_handlers(ns)
    srv, t, port = serve_dashboard(ns, host="0.0.0.0")
    try:
