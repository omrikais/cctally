...
# gate check. That helper is reached by stats migrations 008, 009 AND 010, so
# the two modules migrated earlier were two thirds of one class rather than a
# pair. The violation is masked whenever an earlier module on the same process
# has already rebound the constants, which is the ordering dependence
# `bin/cctally-isolation-sweep` exists to observe.
