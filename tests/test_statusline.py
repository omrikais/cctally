...
class TestRenderStatusline:
    ...
    def test_usage_only_colorizes_by_higher_percent(self):
        """Compact mode still color-bands on the higher of (5h%, 7d%)."""
        inp = ls.StatuslineInput(
            model_display_name="Sonnet 4.5",
            rate_limits_5h_pct=90.0, rate_limits_5h_resets_at=_FIVE_RESETS,
            rate_limits_7d_pct=30.0, rate_limits_7d_resets_at=_SEVEN_RESETS,
        )
        out = ls.render_statusline(
            inp, _make_args(usage_only=True, color=True), _make_inj(), _FIXED_NOW
        )
        assert "\033[31m" in out  # red band (max pct 90 >= 85)
        assert out.endswith("\033[0m")  # segment wrapped + reset
        assert "5h 90% · 7d 30%" in out  # countdowns still omitted
