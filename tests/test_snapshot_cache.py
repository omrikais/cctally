...
#
# Mirrors the `test_reconcile_weekref_*` cluster (same in-memory cache.db +
# mutation_seq helpers). The reconcile stores no domain object — the tests
# use `object()` sentinels keyed by display-tz `YYYY-MM-DD`. Eviction is
# INCLUSIVE (`date_key >= wm_day`) because a day bucket is the closed
# interval `[start, end]`.
# ===========================================================================
