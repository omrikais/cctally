...
Loads ``tests/fixtures/migrations/per-migration/013_create_conversation_sessions/pre.sqlite``
(an existing install at the 012 head: cache migrations 001-012 applied, no
``conversation_sessions_backfill_pending`` flag, no 013 marker), runs the
production 013 handler against a copy, and asserts it arms the flag.

013 is FLAG-ONLY: it sets the DISTINCT
...
_MIGRATION = "013_create_conversation_sessions"
...
def test_pre_fixture_at_012_head_without_flag_or_marker(cctally_module):
    """Sanity: pre.sqlite has 012 applied, has NOT the 013 marker, does NOT carry
    the backfill flag, and already has the conversation_sessions table (created
    by _apply_cache_schema) — the existing-install shape before the rollup
    backfill is armed."""
    ...
    try:
        assert conn.execute(
            ...
            "WHERE name='012_create_conversation_ai_titles'"
        ).fetchone()[0] == 1
        assert conn.execute(
...
def test_post_fixture_has_flag_set_and_marker_stamped(cctally_module):
    """Sanity: post.sqlite carries the backfill flag set to '1' and the 013 marker
    stamped (central dispatcher stamp, #140). The table is STILL empty — the
    recompute is sync-side, not in the flag-only handler."""
...
def test_handler_sets_flag(cctally_module, tmp_path):
    ...
    flag (and only the flag — no data table touched), then stamp the 013 marker."""
